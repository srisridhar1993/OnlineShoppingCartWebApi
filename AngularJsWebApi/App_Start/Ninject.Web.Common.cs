...
namespace AngularJsWebApi.App_Start
{
    ...
    public static class NinjectWebCommon 
    {
        ...
        private static void RegisterServices(IKernel kernel)
        {
            ...
            kernel.Bind<IEmployee>().To<EmployeeManager>().InRequestScope();
            kernel.Bind<IEmployeeDA>().To<EmployeeDA>().InRequestScope();

            kernel.Bind<IProduct>().To<ProductManager>().InRequestScope();
            kernel.Bind<IProductDA>().To<ProductDA>().InRequestScope();

            kernel.Bind<ICustomer>().To<CustomerManager>().InRequestScope();
            kernel.Bind<ICustomerDA>().To<CustomerDA>().InRequestScope();

            kernel.Bind<IOrder>().To<OrderManager>().InRequestScope();
            kernel.Bind<IOrderDA>().To<OrderDA>().InRequestScope();
        }        
    }
}